...
class Pose_Check:
    ...
    def pose_run(self, MEDIA_ROOT, video_file):
        pose = Pose_Check(
            '/home/ubuntu/pose_model/pose_deploy_linevec.prototxt',
            '/home/ubuntu/pose_model/pose_iter_440000.caffemodel',
        )
        ...
        pose_json = {'shoulder':{
                ...
                'unbalance':int(all_count['sh_count'].sum()),
                'none':int(all_count['sh_count'].isnull().sum())
            },
            'pelvis' :{
                'len':len(all_count),
                'balance':len(all_count) - (all_count['pel_count'].sum() + all_count['pel_count'].isnull().sum()),
                'unbalance':int(all_count['pel_count'].sum()),
                'none':int(all_count['pel_count'].isnull().sum())
            },
            'eye' :{
                'len':len(all_count),
                'balance':len(all_count) - (all_count['eye_count'].sum() + all_count['eye_count'].isnull().sum()),
                'unbalance':int(all_count['eye_count'].sum()),
                'none':int(all_count['eye_count'].isnull().sum())
            },
            'all_pose':{
                'len':len(all_count),
                'balance':len(all_count) - (all_count['pose_count'].sum() + all_count['pose_count'].isnull().sum()),
                'unbalance':int(all_count['pose_count'].sum()),
                'none':int(all_count['pose_count'].isnull().sum())
            }}
